...
def shout_with_a_number(a_string, a_number):
    ...
    """
    the_answer = f"{str.upper(a_string)} {a_number}"

    """
    # the_answer = str.upper(a_string) + " " + str(a_number)
    # the_answer = str.upper(a_string) + " % s" % a_number
    # the_answer = str.upper(a_string) + " {}".format(a_number)
    # the_answer = str.upper(a_string) + " " + a_number.__str__()

    There seems to be a million ways to do the same thing, I will 
    probably just use the first one as it is less characters
    """
